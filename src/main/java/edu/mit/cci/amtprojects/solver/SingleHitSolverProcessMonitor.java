...
import edu.mit.cci.amtprojects.solver.SolverPluginFactory.Phase;
 ...
 * Time: 2:55 PM
 */
public class SingleHitSolverProcessMonitor extends BatchProcessMonitor {


    private static Logger logger = Logger.getLogger(SingleHitSolverProcessMonitor.class);

    public SingleHitSolverProcessMonitor(Batch b) {
        super(b);
    }

    public static SingleHitSolverProcessMonitor get(Batch b) {
        try {
            return (SingleHitSolverProcessMonitor) get(b, SingleHitSolverProcessMonitor.class);
    ...
    public void update() throws UnsupportedEncodingException, JSONException {
        ...
        manager.updateHits();
        List<TurkerLog> logs = Collections.emptyList();
    ...
    private void updateRanks(Batch b, SolverTaskModel model, List<TurkerLog> logs) throws JSONException {
        ...
        for (TurkerLog log : logs) {
            ...
            String phase = MturkUtils.extractAnswer(log, "phase");
            if (!SolverPluginFactory.Phase.RANK.name().equals(phase) && !SolverPluginFactory.Phase.INIT.name().equals(phase)) continue;
    ...
    private List<TurkerLog> findCurrentLogs(int currentRound, SolverPluginFactory.Phase currentPhase, List<TurkerLog> logs) {
