 ...
 * Time: 2:55 PM
 */
public class KickballProcessMonitor {
    ...
    private final static Map<Long, KickballProcessMonitor> monitorMap = new HashMap<Long, KickballProcessMonitor>();


    private static Logger logger = Logger.getLogger(KickballProcessMonitor.class);

    private KickballProcessMonitor(Batch b) {
        batchId = b.getId();
        // if (!running) restart();
    }
    ...
    public static KickballProcessMonitor get(Batch b) {
        if (!(monitorMap.containsKey(b.getId()))) {
            monitorMap.put(b.getId(), new KickballProcessMonitor(b));
        }
